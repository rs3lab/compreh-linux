 ...
 * COLD = frag[2] (never addressed; noise floor).  HDR is the byte offset the
 * gadget reads within a frag page, so we time the same 64B line.
 * Generated by: GPT-5 -- versioned the resumed SIG=frag[3] / COLD=frag[2]
 * receive-side experiment state.
 *
 * Indices lowered from 9/5 to 3/2 so the probe's mappability gate
 ...
 * now yields both a clean floor and a high mappable rate.  SIG (frag3) stays 2
 * pages from ARCH (frag1), beyond cross-page prefetch, so the SIG floor is still
 * warmed only speculatively.  This also more faithfully models a real HDS NIC,
 * which delivers every frag mappable rather than only deep-merged ones. */
#define CPOC_HDR        32u
#define CPOC_ARCH_FRAG  1
#define CPOC_SIG_FRAG   3
#define CPOC_COLD_FRAG  2
