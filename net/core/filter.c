...
bool cbpf_poc_inject_enabled = true;
EXPORT_SYMBOL_GPL(cbpf_poc_inject_enabled);
u64 cbpf_poc_stale_hist[256];
EXPORT_SYMBOL_GPL(cbpf_poc_stale_hist);
 ...
 * before the socket filter and timed immediately after it on the same CPU.
 *
 * Generated by: GPT-5. */
#define CPOC_HDR             32u
...
/* One-time calibration off a kernel scratch line. min-of-N is valid because
 * this line is deterministically warm (touch) or cold (clflush). */
...
static int cbpf_poc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "stale=0x%02x inject=0x%02x inject_enabled=%d stack_off=%d slot=%u\n",
		   (unsigned int)cbpf_poc_m0_stale,
		   (unsigned int)cbpf_poc_stale_value,
		   cbpf_poc_inject_enabled,
	...
	seq_puts(m, "stale_hist");
	for (i = 0; i < ARRAY_SIZE(cbpf_poc_stale_hist); i++) {
		u64 n = READ_ONCE(cbpf_poc_stale_hist[i]);

		if (n)
			seq_printf(m, " %02x:%llu", i, n);
	}
	seq_putc(m, '\n');
	return 0;
}
 ...
 *   'i' / 'n' -- enable / disable explicit stale-byte injection
 * Generated by: GPT-5. */
			      ...
			      size_t len, loff_t *off)
{
	...
	if (c == 'c') {
		...
		cbpf_poc_inject_enabled = true;
	} else if (c == 'n') {
		cbpf_poc_inject_enabled = false;
	} else if (c == 'v') {
