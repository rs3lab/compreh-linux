 ...
 * SIG  = frag[3] (speculative SSB P[X] target, warmed iff stale bit = 1);
 ...
 * Generated by: GPT-5 -- versioned the resumed SIG=frag[3] receive-side
 * experiment state and moved COLD back to frag[5].
 *
 * SIG is lowered from 9 to 3 so the signal's mappability gate
 * (nr_frags > CPOC_SIG_FRAG) needs only nr_frags >= 4, not >= 10.  This
 * decouples mappable-frag throughput from the GRO-merge depth (a deep merge
 * needs a large rcvbuf, whose deep queue lengthens the RX->getsockopt window and
 * destroys the userspace floor -- see STOCK_CHANNEL_ANALYSIS 7i).  A small rcvbuf
 * now yields both a clean floor and a high mappable rate.  COLD stays away from
 * SIG because the primed userspace readout can warm adjacent frag pages; the
 * frag2/frag3 layout raised the null SIG floor in the floor_sweep diagnostic.
 * This also more faithfully models a real HDS NIC, which delivers every frag
 * mappable rather than only deep-merged ones. */
#define CPOC_HDR        32u
#define CPOC_ARCH_FRAG  1
#define CPOC_SIG_FRAG   3
#define CPOC_COLD_FRAG  5
#define CPOC_MAX_FRAG   ((CPOC_SIG_FRAG > CPOC_COLD_FRAG) ? CPOC_SIG_FRAG : CPOC_COLD_FRAG)
