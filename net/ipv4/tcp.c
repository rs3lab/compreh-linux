...
/* Generated by: GPT-5 -- obsolete PoC TCP zerocopy hint handling removed. */
static int tcp_zerocopy_vm_insert_batch_error(unsigned long pages_remaining,
					      u32 *length,
					      struct tcp_zerocopy_receive *zc,
					      int err)
{
					...
					struct tcp_zerocopy_receive *zc)
{
	...
	*address += bytes_mapped;
	...
	/* Error: rollback the state for failed inserts. */
	return tcp_zerocopy_vm_insert_batch_error(pages_remaining, length, zc,
						  err);
}
				...
				struct scm_timestamping_internal *tss)
{
	...
	while (length + PAGE_SIZE <= zc->length) {
		...
		if (zc->recv_skip_hint < PAGE_SIZE) {
			...
			{
				static atomic_t cbpf_hds_diag_n = ATOMIC_INIT(0);

				if (atomic_fetch_add(1, &cbpf_hds_diag_n) < 8) {
					int i;
					...
					for (i = 0; i < skb_shinfo(skb)->nr_frags; i++) {
						...
						struct page *p = skb_frag_page(f);

						pr_info("cbpf_hds_diag:  [%d] off=%u sz=%u comp=%d map=%d\n",
							i, skb_frag_off(f), skb_frag_size(f),
							PageCompound(p), !!p->mapping);
					}
				}
			}
#endif
